...
%-lc-sd.o: %.c
	cc65816 --core=65816 --target=Foenix --code-model=large --data-model=small -O2 -Iinclude --list-file=$(@:%.o=obj/%.lst) -o obj/$@ $<

%-lc-md.o: %.s
	as65816 --core=65816 --target=Foenix --code-model=large --data-model=medium --list-file=$(@:%.o=obj/%.lst) -o obj/$@ $<

%-lc-md.o: %.c
	cc65816 --core=65816 --target=Foenix --code-model=large --data-model=medium -O2 -Iinclude --list-file=$(@:%.o=obj/%.lst) -o obj/$@ $<
